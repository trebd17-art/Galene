...
function updateXP(){
  const capped = Math.min(mindPoints, 100);
  document.getElementById("xpFill").style.width = capped + "%";
  document.getElementById("xpText").textContent = `${mindPoints} / 100 Mind Points`;
  document.getElementById("levelName").textContent = getLevelName(mindPoints);
  document.getElementById("activitiesText").textContent = completedActivities;
  document.getElementById("streakText").textContent = completedActivities > 0 ? "1 day" : "0 days";
}
...
function buildPlan(){
  ...
  currentProfile = { improvements, challenge, time, hope, userType, path, mindType };

  document.getElementById("profileCard").innerHTML =
    `<strong>Your profile:</strong><br>
    • User type: ${userType}<br>
    • Wants to improve: ${improvementText}<br>
    • Biggest challenge: ${challenge}<br>
    • Desired feeling: ${hope}<br>
    • Time available: ${time} per day`;

  document.getElementById("mindTypeCard").innerHTML =
    `<strong>${mindType.name}</strong><p>${mindType.description}</p>`;

  document.getElementById("pathCard").innerHTML =
    `<strong>Primary Path: ${path.title}</strong><br>
    Supporting Focus: ${supportingPaths.length ? supportingPaths.join(", ") : "Steady daily habits"}<br><br>
    This path is designed to help you feel more <strong>${hope}</strong> while working through <strong>${challenge}</strong>.`;

  document.getElementById("dailyPlanCard").innerHTML =
    `<strong>Today's Focus: ${path.name}</strong><br>
    Today's Goal: Take one small action that helps you steady your mind.<br><br>
    <strong>Your ${time} routine:</strong>
    <ol>
      <li>${path.daily[0]}</li>
      <li>${path.daily[1]}</li>
      <li>${path.daily[2]}</li>
