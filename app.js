...
/* v41 Dynamic Home Plan CTA */
function handleHomePlanCTA(){
  if(typeof hasBuiltPlan === 'function' && hasBuiltPlan()){
    showScreen('profile');
  } else {
    showScreen('assessment');
  }
...
function refreshHomePlanCTA(){
  ...
  if(typeof hasBuiltPlan === 'function' && hasBuiltPlan()){
    ...
    if(card) card.textContent = 'Your personalized plan is ready. Continue where you left off.';
  } else {
    btn.textContent = 'Build My Plan →';
    if(card) card.textContent = 'Complete your assessment to create a personalized plan.';
  }
}

document.addEventListener('DOMContentLoaded', function(){
  setTimeout(refreshHomePlanCTA, 300);
});


/* v42 Auto-redirect after plan build */
(function(){
  ...
  function afterPlanBuiltRedirect(){
    try {
      if(typeof saveSteadyMindData === "function") saveSteadyMindData();
      if(typeof refreshHomePlanCTA === "function") refreshHomePlanCTA();
      if(typeof updatePlanStatus === "function") updatePlanStatus();
    } catch(e){}
  ...
  document.addEventListener("DOMContentLoaded", function(){
    document.querySelectorAll("button").forEach(function(btn){
      ...
      if(txt.includes("build my plan") || txt.includes("build plan")){
...
/* v43 Correct Build/Continue Plan Card Logic */
function steadyMindPlanExists(){
  try {
    ...
    // fallback: if profile details exist after assessment
    if(typeof selectedMindType !== "undefined" && selectedMindType) return true;
  } catch(e){}
  return false;
}

function handleHomePlanCTA(){
  if(steadyMindPlanExists()){
    showScreen("profile");
  } else {
    showScreen("assessment");
  }
...
function refreshBuildContinuePlanCard(){
  var heading = document.getElementById("homePlanHeading");
  var subheading = document.getElementById("homePlanSubheading");
  var desc = document.getElementById("homePlanDescription");
  var btn = document.getElementById("homePlanCTA");

  if(steadyMindPlanExists()){
    if(heading) heading.innerHTML = "Continue My<br>Plan";
    if(subheading) subheading.textContent = "Return to your personalized path";
    if(desc) desc.textContent = "Your SteadyMind profile is ready. Continue your daily calm, focus, and confidence plan.";
    if(btn){
      btn.textContent = "Continue My Plan →";
      btn.onclick = function(){ showScreen("profile"); };
    }
  } else {
    if(heading) heading.textContent = "Build Plan";
    if(subheading) subheading.textContent = "Build your personalized path";
    if(desc) desc.textContent = "Answer a few questions so SteadyMind can guide your daily calm, focus, and confidence plan.";
    if(btn){
      btn.textContent = "Build My Plan →";
      btn.onclick = function(){ showScreen("assessment"); };
    }
  }
}

document.addEventListener("DOMContentLoaded", function(){
  setTimeout(refreshBuildContinuePlanCard, 250);
});

(function(){
  var previousShowScreen = typeof showScreen === "function" ? showScreen : null;
  if(previousShowScreen){
    showScreen = function(screenId){
      previousShowScreen(screenId);
  ...
  var previousBuildMyPlan = typeof buildMyPlan === "function" ? buildMyPlan : null;
  if(previousBuildMyPlan){
    buildMyPlan = function(){
      var result = previousBuildMyPlan.apply(this, arguments);
