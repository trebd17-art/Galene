...
function loadAppData() {
  ...
  if (savedNotifications) {
    appData.notifications = { ...appData.notifications, ...savedNotifications };
  }

  // Only count a plan as built when the actual saved plan exists.
...
function getPlanPathTitle(plan) {
  if (!plan || !plan.path) return "Calm Foundations";
  if (typeof plan.path === "string") return plan.path;
  return plan.path.title || plan.path.name || "Calm Foundations";
}

function getPlanPathName(plan) {
  if (!plan || !plan.path) return "Calm & Anxiety";
  if (typeof plan.path === "string") return plan.path;
  return plan.path.name || plan.path.title || "Calm & Anxiety";
}

function getPlanDailySteps(plan) {
  if (plan && plan.path && typeof plan.path === "object" && Array.isArray(plan.path.daily)) {
    return plan.path.daily;
  }
  return ["60-second breathing reset", "Grounding exercise", "Name one thing you can control today"];
}
...
function renderHome() {
  ...
  if(!appData.planBuilt || !appData.savedPlan) {
    ...
    if(status) status.textContent = `Plan ready: ${getPlanPathTitle(plan)} · ${plan.time} per day`;
...
function renderSavedPlan(){
  ...
  if(pathCard) pathCard.innerHTML = `<strong>Primary Path: ${getPlanPathTitle(plan)}</strong><br><br>Supporting Focus: ${plan.supportingPaths && plan.supportingPaths.length ? plan.supportingPaths.join(", ") : "Steady daily habits"}<br><br>This path helps you feel more <strong>${plan.hope}</strong> while working through <strong>${plan.challenge}</strong>.`;
